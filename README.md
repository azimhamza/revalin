...
- `SHIPPO_ORIGIN_NAME`, `SHIPPO_ORIGIN_PHONE`, `SHIPPO_ORIGIN_STREET1`, `SHIPPO_ORIGIN_CITY`, `SHIPPO_ORIGIN_STATE`, `SHIPPO_ORIGIN_ZIP`, `SHIPPO_ORIGIN_COUNTRY` (required to activate Shippo live rates / labels)
- `SHIPPO_ORIGIN_EMAIL` (optional sender email for Shippo; falls back to `SHIPPING_LABEL_EMAIL`, the first `SHIPPING_LABEL_EMAILS` entry, or `LOOPS_SHIPPING_LABEL_EMAIL`)
...
- `SHIPPO_CARRIER_ACCOUNT_IDS` (optional comma-separated Shippo carrier account IDs; when set, quotes are limited to these accounts instead of all Shippo default/master accounts)
- `SHIPPO_US_CARRIER_ACCOUNT_IDS` (optional comma-separated Shippo carrier account IDs for US destinations; use this for your connected UPS account)
- `SHIPPO_CA_CARRIER_ACCOUNT_IDS` (optional comma-separated Shippo carrier account IDs for Canadian destinations)
- `SHIPPO_INTERNATIONAL_CARRIER_ACCOUNT_IDS` or `SHIPPO_INTL_CARRIER_ACCOUNT_IDS` (optional comma-separated Shippo carrier account IDs for non-domestic international destinations)
...
- `SHIPPO_CHECKOUT_SHIPPING_MARKUP_AMOUNT` (optional; defaults to `3`, added to customer-facing checkout rates; admin label-preview rates remain the raw carrier cost)
- `SHIPPO_FALLBACK_TO_SHIPENGINE_ON_NO_RATES=true` (optional temporary cutover flag; when Shippo is configured but returns no checkout rates or errors, checkout tries legacy ShipEngine rates before giving up)
- Shippo checkout/admin label rates prefer UPS when returned by Shippo. Canadian destinations fall back to Canada Post when no UPS rate is available, so domestic Canadian checkout does not dead-end. Checkout shows up to three generic customer options (`Standard Shipping`, `Priority Shipping`, `Express Shipping`) while preserving the underlying Shippo rate IDs for label purchase.
- `CHECKOUT_SHIPMENT_PROTECTION_FEE_AMOUNT` (optional; defaults to `5`, added when the customer selects shipment protection)
- `SHIPPO_INSURANCE_CONTENT` (optional; defaults to `laboratory supplies`, sent to Shippo as the protected shipment content)
- `SHIPPO_INSURANCE_MAX_VALUE_AMOUNT` (optional; defaults to `10000`, caps the insured product value requested from Shippo)
